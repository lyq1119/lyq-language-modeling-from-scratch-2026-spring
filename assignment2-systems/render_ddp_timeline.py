"""Render a Gantt-style timeline from nsys SQLite exports comparing the naive
DDP and the overlapped DDP implementations.

Shows, per rank: the NVTX phase bands (forward/backward/comm/optimizer) in the
background, and the CUDA kernels executed on the GPU on top — NCCL kernels in
red, everything else in gray.  The key visual is whether red (communication)
kernels appear *after* the backward band (naive) or *inside* it (overlap).
"""
...
# Register a CJK-capable font so Chinese labels render correctly.
...
PHASES = ("forward", "backward", "comm", "optimizer")
PHASE_COLORS = {"forward": "#9ecae1", "backward": "#fdae6b", "comm": "#d8b7f4", "optimizer": "#a1d99b"}
NS = 1e6  # ns -> ms
...
def load(path: Path):
    ...
    cur = conn.cursor()
    cur.execute("SELECT globalPid, pid FROM PROCESSES WHERE name='python'")
...
def rank_phases_kernels(pyprocs, kernels, nvtx, gp_pid):
    """Return (phases_sorted, kernels) for a given (globalPid, pid)."""
    gp, pid = gp_pid
    phases = []
    for phase in PHASES:
...
def rank_processes(pyprocs, kernels, nvtx):
    """Return list of (globalPid, pid) for processes that actually have kernels."""
...
def draw_lane(ax, phases, klist, y0, y1):
    ...
    # NVTX phase bands as semi-transparent horizontal bars
...
def render(db: Path, title: str, ax):
    ...
    # Use the LAST occurrence of the optimizer as the end of the measured step.
    ...
    # Use the SECOND-to-last 'forward' as start (last is the measured step).
    forwards = []
    for gp, pid in ranks:
        phases, _ = rank_phases_kernels(pyprocs, kernels, nvtx, (gp, pid))
        for s, e, phase in phases:
...
def main() -> None:
    traces = [
        ("profiles/ddp_naive.sqlite", "naive（反向传播后逐参数 all-reduce）"),
        ("profiles/ddp_overlap.sqlite", "overlap（反向传播中异步 all-reduce）"),
    ]
    fig, axes = plt.subplots(2, 1, figsize=(12, 6))
    for ax, (db, title) in zip(axes, traces):
        render(Path(db), title, ax)
    handles = [
        Patch(facecolor="#9ecae1", label="forward"),
        Patch(facecolor="#fdae6b", label="backward"),
        Patch(facecolor="#d8b7f4", label="comm"),
    ...
    fig.savefig(out / "ddp_timeline.png", dpi=150)
    print("saved figures/ddp_timeline.png")
